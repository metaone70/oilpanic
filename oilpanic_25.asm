...
// read keyboard AND joystick input with debouncing and delay

		lda fire_active					// if there is fire on the upper part
		beq !no_fire+					// don't let the player move
		rts

!no_fire:
		lda lady_spill_active       	// if oil is spilling over lady
		beq !no_lady_spill+						// don't let the player move
		rts

!no_lady_spill:
		lda gent_spill_active       	// if oil is spilling over gent
		beq !no_gent_spill+						// don't let the player move
		rts

!no_gent_spill:
		//we have no spills, so can continue to check the controls
		
		lda joy_delay_timer				// Check if we're still in movement delay
		beq !check_input+               // Timer is 0, can move
		dec joy_delay_timer             // Timer active, decrement and skip
		jmp !update_joy+

!check_input:
		lda $dc00                       // Read joystick port 2
		sta joy_current                 // Store current state
		
		and #$04                        // Check LEFT bit (bit 2)
		bne !not_joy_left+              // Not pressed (bit is 1)
		
		lda joy_previous				// LEFT is pressed - check if it was pressed last frame
		and #$04
		bne moveleft               		// Changed from beq to bne, jump if NEW press
		jmp !joy_still_held+       		// Was held, go to auto-repeat check
		
!not_joy_left:
		lda joy_current					// Check if joystick RIGHT
		and #$08                        // Check RIGHT bit (bit 3)
		bne !joy_released+              // Not pressed
		
		lda joy_previous				// RIGHT is pressed - check if it was pressed last frame
		and #$08
		bne moveright             		// Changed from beq to bne, jump if NEW press
		jmp !joy_still_held+       		// Was held, go to auto-repeat check

!joy_released:
		// Joystick is released - allow immediate next move
		lda #$00
		sta joy_delay_timer
		jmp !check_keyboard+

!joy_still_held:
		inc joy_hold_counter			// Joystick still held - check if we should do auto-repeat
		lda joy_hold_counter
		cmp #$0f                        // After 15 frames (~0.25 sec), start auto-repeat
		bcs !do_repeat+            		// Not yet
		jmp !check_keyboard+			// Check keyboard instead

!do_repeat:		
		// Auto-repeat: reset hold counter and check direction
		lda #$00
		sta joy_hold_counter
		
		lda joy_current
		and #$04
		beq moveleft                    // Still holding left
		lda joy_current
		and #$08
		beq moveright                   // Still holding right

!check_keyboard:
		jsr GETIN						// No joystick input - check keyboard
		cmp leftkey 
		beq moveleft
		cmp rightkey 
		beq moveright
...
moveleft:
		...
		lda #$03                        // Set delay (3 frames between moves)
		sta joy_delay_timer
		lda #$00
		sta joy_hold_counter
		jmp !update_joy+
...
!update_joy:
		lda joy_current					// Store current joystick state for next frame
		sta joy_previous
...
joy_current:    				.byte $ff			// for joystick debouncing
joy_previous:    				.byte $ff			// for joystick debouncing
joy_delay_timer:    			.byte $00			// counters for joystick
joy_hold_counter:   			.byte $00			// otherwise too fast, js isread every frame
